...
//// Inside WebSocketMailboxFlusher.java
//
//    @Async
//    @EventListener
...
//
//        // 🟢 FIX: Listen for the new personal inbox subscription
//        if (!destination.startsWith("/topic/shadow-user-")) return;
//
//        String username = destination.substring("/topic/shadow-user-".length()).trim().toLowerCase();
//
//        // 🟢 FIX: Fetch pending messages for the USER, not a specific room
//        List<OfflineMessage> pending =
//                offlineMessageRepository.findByRecipientUsernameOrderByTimestampAsc(username);
//
...
//            } catch (Exception e) {
//                log.error("Failed to deliver mailbox message {} to {}: {}", msg.getId(), username, e.getMessage());
//                break;
//            }
//        }
//
//        if (!delivered.isEmpty()) {
//            offlineMessageRepository.deleteAllById(delivered); // or deleteByIdIn if you have a custom query
//            log.info("Flushed {} vaulted messages for {}", delivered.size(), username);
//        }
//    }
//}
