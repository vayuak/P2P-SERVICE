...
public class ShieldHandshakeFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        // 🟢 FIXED: Strip the invisible YAML spacing from the Gateway!
        ...
        if (EXPECTED_KEY.equals(incomingShieldKey) && EXPECTED_GATEWAY_SECRET.equals(incomingGatewaySecret)) {
            ...
            response.getWriter().write("{\"error\": \"GHOST_SHIELD_VIOLATION\", \"message\": \"Direct microservice access is denied. Connect via the API Gateway tunnel.\"}");
        }
    }
}